...
from fastPTA.signals import SMBBH_parameters, CGW_SIGW_parameters, CGW_LN_parameters, CGW_BPL_parameters
...
def HD_constraints(
    ...
    generate_catalog_kwargs={"n_pulsars": 100, "save_it": True, **mockSKA10},
    add_HD_prior=True,
...
def get_constraints(
    ...
    MCMC_kwargs={},
    get_tensors_kwargs={},
    generate_catalog_kwargs={"n_pulsars": 10, "save_it": True, **mockSKA10},
    len_fisher_data=10000,
    parameter_labels=[
        r"$\alpha_{\rm PL}$",
        r"$n_{\rm T}$",
    ...
    # )
    print("-- Done --\n")
    return fisher_data, len_fisher_data, ranges
...
if __name__ == "__main__":
    ...
    #     np.linspace(-13, -7, 15),
    #     n_frequencies=100,
    #     generate_catalog_kwargs={"n_pulsars": 400, "save_it": True, **mockSKA10},
    #     get_tensors_kwargs={
    ...
    fisher_data100, len_fisher_data100, ranges100 = get_constraints(
       "power_law_SIGW",
    ...
    fisher_data400, len_fisher_data400, ranges400 = get_constraints(
       "power_law_SIGW",
        np.concatenate([SMBBH_parameters, CGW_SIGW_parameters]),
        T_obs_yrs=10,
        n_frequencies=200,
    ...
    datasets = [fisher_data100, fisher_data400]
    weights = [np.ones(len_fisher_data100), np.ones(len_fisher_data400)]
    smooth = [1.0, 1.0]
    ranges = ranges400

    plot_corner(
            datasets,
            colors=[my_colormap["red"], my_colormap["blue"]],
            chain_labels=["100", "400"],   
            weights=weights,
            smooth=smooth,
            labels=[
            "$\\alpha_{PL}$",
